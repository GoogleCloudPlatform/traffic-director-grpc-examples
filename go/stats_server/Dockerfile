# Dockerfile for building the stats server. To build the image, run the
# following command from the traffic-director-grpc-examples directory:
# docker build -t <TAG> -f go/stats_server/Dockerfile .
...
WORKDIR go/stats_server
RUN go build -o stats_server -tags osusergo,netgo main.go
...
FROM alpine
COPY --from=build /traffic-director-grpc-examples/go/stats_server/stats_server .
