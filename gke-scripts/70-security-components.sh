...
function create_security_components {
  envsubst < ServerMtlsPolicy.yaml | gcloud network-security \
    server-tls-policies import ${SERVER_MTLS_POLICY_NAME} --location=global

  envsubst < EndpointPolicy.yaml | gcloud network-services \
    endpoint-policies import ${ENDPOINT_MTLS_POLICY_NAME} --location=global

  envsubst < ClientMtlsPolicy.yaml | gcloud network-security \
  ...
  do
    gcloud compute backend-services export ${bs} --global \
      --destination=/tmp/${bs}.yaml

    envsubst < ClientSecuritySettings.yaml | cat /tmp/${bs}.yaml - \
      >/tmp/${bs}-new.yaml

    gcloud compute backend-services import ${bs} --global \
...
function delete_server_security_components {
  gcloud network-services endpoint-policies delete ${ENDPOINT_MTLS_POLICY_NAME} --location=global -q
  gcloud network-security server-tls-policies delete ${SERVER_MTLS_POLICY_NAME} --location=global -q
}

function delete_client_security_components {
  gcloud network-security client-tls-policies delete ${CLIENT_MTLS_POLICY_NAME} --location=global -q
}
